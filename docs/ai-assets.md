...
| 구분 | 내용 |
|---|---|
| **게임 내 외부 저작물** | **0건.** 피치·선수 칩·아이콘·파비콘은 전부 CSS/SVG로 직접 작성했고, 사운드는 오디오 파일 없이 Web Audio API로 합성한다(`src/game/audio.ts`). 웹폰트를 불러오지 않으며 시스템 기본 글꼴만 쓴다 |
| **AI 생성물** | 배경 이미지·영상은 Higgsfield **유료 구독(Plus)** 계정에서, 시연영상 BGM은 Suno **유료 구독(Pro)** 계정에서 생성했다. 각 자산의 모델·설정·프롬프트 원문은 아래 항목별로 기록되어 있다 |
| **AI 생성물 사용처** | 이미지·영상은 게임 배경과 시연영상에, 음원은 **시연영상에만** 쓴다 |
| **실존 선수 실명·등번호·포지션** | 대회 요강이 명시적으로 허용한 범위(요강 「데이터 활용」 항목). 출처는 `docs/sources.md`에 전수 기록 |
| **선수 개인 능력치** | **만들지 않았다.** 배치 적합도 3단계(적합/인접/부적합)만 쓴다. 특정 선수의 기량을 수치로 평가하지 않기 위해서다 |
| **감독 실명** | **쓰지 않는다.** "그날의 벤치"로만 지칭하며, AI 결산 출력에도 실명·판정 어휘를 거르는 후처리 필터가 있다(`api/verdict.ts`) |
| **초상권** | 생성 프롬프트에 `No people` 을 넣어 사람이 나오지 않게 했고, 결과물을 프레임 단위로 확인했다. 경기 화면의 상대팀은 번호도 이름도 없는 점으로만 그린다 |
| **상표권** | 생성 프롬프트에 `no logos, no brand marks, no banners` 를 넣었고, FIFA·구단 로고나 유니폼 디자인이 보이는 컷은 채택하지 않았다 |

> 유료 구독 플랜에서 생성했다는 사실과, 각 서비스 약관상 상업적 이용 조건은 별개다. **제출 전 두 서비스의 현재 약관을 직접 확인할 것.**
